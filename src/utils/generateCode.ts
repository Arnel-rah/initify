...
function buildReadme(config: ProjectConfig): string {
  ...
  const dev     = pm === "npm" ? "npm run dev" : pm === "yarn" ? "yarn dev" : "pnpm dev"
...
export function generateFiles(config: ProjectConfig): GeneratedFile[] {
  ...
  if (isNotNextNuxt) {
    ...
    files.push({ name: "index.html",     content: buildIndexHtml(config),  language: "html" })
  }

  if (config.styling === "tailwind") {
    files.push({ name: "tailwind.config.js", content: buildTailwindConfig(config), language: "javascript" })
    files.push({ name: "postcss.config.js",  content: buildPostcssConfig(),        language: "javascript" })
  }

  if (isTs) {
    files.push({ name: "tsconfig.json", content: buildTsConfig(config), language: "json" })
    if (isNotNextNuxt) {
      files.push({ name: "src/vite-env.d.ts", content: buildEnvDts(), language: "typescript" })
    }
  }

  if (isVue) {
    files.push({ name: `src/main.${srcExt}`, content: buildMainVue(),       language: "typescript" })
    files.push({ name: "src/App.vue",         content: buildAppTsx(config), language: "html" })
  } else if (!isNuxt) {
    files.push({ name: `src/main.${srcExt}`,  content: buildMainTsx(config), language: "typescript" })
    files.push({ name: `src/App.${srcExt}`,   content: buildAppTsx(config),  language: "typescript" })
  }

  files.push({ name: "src/index.css", content: buildIndexCss(config), language: "css" })
